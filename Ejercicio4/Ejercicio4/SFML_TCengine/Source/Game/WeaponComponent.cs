    ...
    public class WeaponComponent : BaseComponent
    {
        ...
        public void Shoot()
        {
            if (m_TimeToShoot <= 0.0f)
            {
                ...
                FollowMouseComponent mouseFollowerComponent = Owner.GetComponent<FollowMouseComponent>();
                Vector2f _forward = mouseFollowerComponent.Forward;

                bulletActor.AddComponent<BulletComponent>();
                bulletActor.AddComponent<LinealMovementComponent>(700f, _forward);
